...
// -----------------------------------------------------------------------
static void parse_command(char inputBuffer[], int length, struct output_commands* output);

void get_command(char inputBuffer[], int size, struct output_commands* output, size_t* output_length)
{
    ...
    *output_length = 0;
    ...
    for(tmp = strtok_r(inputBuffer, "|", &ctx); tmp; tmp = strtok_r(NULL, "|", &ctx)) {
        ...
        parse_command(tmp, len, &output[*output_length]);
        if(len_modified) {
            tmp[len - 1] = '\0';
        }
        if(output[*output_length].args[0] != NULL) {
            (*output_length)++;
        }
    }
...
static void parse_command(char inputBuffer[], int length, struct output_commands* output){
    ...
    { 
        ...
        case '\t' :               /* argument separators */
            if(start != -1)
            {
                output->args[ct] = &inputBuffer[start];    /* set up pointer */
                ct++;
            }
        ...
        case '\n':                 /* should be the final char examined */
            if (start != -1)
            {
                output->args[ct] = &inputBuffer[start];     
                ct++;
            }
            inputBuffer[i] = '\0';
            output->args[ct] = NULL; /* no more arguments to this command */
            break;
    ...
    output->args[ct] = NULL; /* just in case the input line was > MAXLINE */
}
